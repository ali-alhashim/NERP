...
if(isset($_GET["currentPage"]))
{
  ...
  $sql = "select id, name, contact_name, email, website, logo, mobile, telephone  FROM suppliers $SearchKeyWord limit $startFromPageNo, $showPerPage ;";
  ...
  while( $SqlRow = $SQLresult->fetch(PDO::FETCH_ASSOC))
  {
   echo('<form action="productsList.php" method="post">');
    ...
    echo('<td> '.$SqlRow['contact_name'].'</td>');
    echo('<td> '.$SqlRow['email'].' </td>');
    echo('<td> '.$SqlRow['website'].'</td>');
    echo('<td> '.$SqlRow['mobile'].'</td>');
    echo('<td><a href="'.$SqlRow['logo'].'" target="_blank"> <img src="'.$SqlRow['logo'].'" alt="photo" width="200px"/></a></td>');
    echo('<td> '.$SqlRow['telephone'].'</td>');
...
?>
              ...
              <form action="suppliersList.php" method="GET">
